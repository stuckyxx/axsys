...
describe("company users page security boundaries", () => {
  ...
  it("ships accessible responsive user management without persistent passwords", async () => {
    ...
    expect(reset).toContain('name="reasonCode"')
    expect(reset).toContain("ADMIN_RESET_SECURITY_INCIDENT")
    expect(reset).toContain("retry.current = payload")
    expect(ui).toContain("version: user.version")
    expect(ui).toContain("max-h-[96dvh]")
    expect(form).toContain("sticky bottom-0")
    expect(reset).toContain("h-dvh")
    expect(reset).toContain("max-h-dvh")
    expect(reset).toContain("overflow-y-auto")
    expect(reset).toContain("sticky bottom-0")
  })
})
