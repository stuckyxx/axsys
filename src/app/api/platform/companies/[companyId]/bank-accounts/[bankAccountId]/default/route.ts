...
import { listPlatformBankAccounts, setDefaultBankAccount } from "@/modules/bank-accounts/server/bank-account-service"
...
const inputSchema = z.object({ version: z.int().positive() }).strict()
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const { version } = inputSchema.parse(await parseBankMutationJson(request))
    return withNoStore(Response.json(await setDefaultBankAccount({
      actorUserId: context.userId, sessionId: context.sessionId,
      companyId, bankAccountId, version, correlationId,
    })))
  } catch (error) {
