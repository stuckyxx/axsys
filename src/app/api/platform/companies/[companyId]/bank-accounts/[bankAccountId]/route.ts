...
import { updateBankAccountSchema } from "@/modules/bank-accounts/schemas/bank-account-schemas"
import { listPlatformBankAccounts, updateBankAccount } from "@/modules/bank-accounts/server/bank-account-service"
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    return withNoStore(Response.json(await updateBankAccount({
      actorUserId: context.userId, sessionId: context.sessionId,
      companyId, bankAccountId, correlationId, input,
    })))
  } catch (error) {
    if (error instanceof ApiError && error.code === "VERSION_CONFLICT") {
      try {
        ...
        const companyId = idSchema.parse(path.companyId)
        const bankAccountId = idSchema.parse(path.bankAccountId)
        const current = (await listPlatformBankAccounts({ context, companyId }))
          .find(({ id }) => id === bankAccountId)
