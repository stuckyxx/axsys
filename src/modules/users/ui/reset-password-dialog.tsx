...
export function ResetPasswordDialog({ membershipId, displayName, onClose }: { membershipId: string; displayName: string; onClose: () => void }) {
  ...
  return <><DialogPrimitive.Root open onOpenChange={(open) => { if (!open) close() }}><DialogPrimitive.Portal><DialogPrimitive.Overlay className="fixed inset-0 z-40 bg-background/80 backdrop-blur-sm" /><DialogPrimitive.Content onEscapeKeyDown={(event) => { if (pending) event.preventDefault() }} onPointerDownOutside={(event) => { if (pending) event.preventDefault() }} className="fixed bottom-0 left-1/2 z-40 w-full max-w-lg -translate-x-1/2 border border-border bg-card p-5 shadow-2xl outline-none sm:bottom-auto sm:top-1/2 sm:-translate-y-1/2 sm:rounded-2xl sm:p-7"><header className="flex justify-between gap-4"><div className="flex gap-3"><KeyIcon size={22} className="text-primary" weight="duotone" /><div><DialogPrimitive.Title className="font-semibold">Redefinir senha</DialogPrimitive.Title><DialogPrimitive.Description className="mt-1 text-sm text-muted-foreground">{displayName}</DialogPrimitive.Description></div></div><Button variant="ghost" size="icon" onClick={close} disabled={pending} aria-label="Fechar"><XIcon /></Button></header>{done ? <div className="mt-6"><p role="status" className="border-l-2 border-primary pl-4 text-sm">Senha provisória atualizada. O usuário deverá trocá-la no próximo acesso.</p><Button className="mt-6 w-full" onClick={close}>Concluir</Button></div> : <form ref={formRef} onSubmit={submit} className="mt-6 space-y-5"><fieldset disabled={pending} className="space-y-5"><div className="space-y-2"><Label htmlFor="reset-password">Nova senha provisória</Label><Input id="reset-password" name="temporaryPassword" type="password" autoComplete="new-password" minLength={12} maxLength={128} required /></div><div className="space-y-2"><Label htmlFor="reset-confirmation">Confirme a senha</Label><Input id="reset-confirmation" name="passwordConfirmation" type="password" autoComplete="new-password" minLength={12} maxLength={128} required /></div><div className="space-y-2"><Label htmlFor="reset-reason">Motivo administrativo</Label><select id="reset-reason" name="reasonCode" required defaultValue="ADMIN_RESET_USER_REQUEST" className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm"><option value="ADMIN_RESET_USER_REQUEST">Solicitação do usuário</option><option value="ADMIN_RESET_ACCESS_RECOVERY">Recuperação de acesso</option><option value="ADMIN_RESET_SECURITY_INCIDENT">Incidente de segurança</option><option value="ADMIN_RESET_ADMINISTRATIVE_CORRECTION">Correção administrativa</option></select></div></fieldset><div aria-live="polite" className="min-h-5 text-sm">{error ? <p role="alert" className="text-destructive">{error}</p> : null}</div><div className="flex justify-end gap-2 border-t border-border pt-4"><Button type="button" variant="ghost" onClick={close} disabled={pending}>Cancelar</Button><Button type="submit" disabled={pending}>{pending ? "Redefinindo..." : "Redefinir senha"}</Button></div></form>}</DialogPrimitive.Content></DialogPrimitive.Portal></DialogPrimitive.Root><ReauthenticationDialog open={reauthOpen} onOpenChange={(open) => { setReauthOpen(open); if (!open) retry.current = null }} onConfirmed={async () => { const payload = retry.current; if (payload) await perform(payload) }} /></>
}
