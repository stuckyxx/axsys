...
export function CompanyUsersPage({ initialUsers, initialNextCursor, currentMembershipId, initialQuery, initialCursor, initialPreviousCursor }: {
  ...
  return <div className="mx-auto w-full max-w-[1400px] space-y-7">
    ...
    <nav aria-label="Paginação do diretório" className="flex justify-between gap-3"><Button className="min-h-11" variant="ghost" onClick={previous} disabled={pending || cursorHistory.length === 0}>Página anterior</Button><Button className="min-h-11" variant="outline" onClick={next} disabled={pending || nextCursor === null}>Próxima página</Button></nav>
...
function Actions({ self, onEdit, onReset }: { self: boolean; onEdit: () => void; onReset: () => void }) { return <div className="flex justify-end gap-1"><Button className="size-11" variant="ghost" size="icon" onClick={onEdit} disabled={self} aria-label={self ? "Você não pode alterar o próprio acesso" : "Editar acesso"}><PencilSimpleIcon /></Button><Button className="size-11" variant="ghost" size="icon" onClick={onReset} disabled={self} aria-label={self ? "Você não pode redefinir a própria senha" : "Redefinir senha"}><KeyIcon /></Button></div> }
